...
var runStyl = function(stylOptions, trueOptions) {
  ...
  if (trueOpts.styl) {
    var styl = trueOpts.styl;
  } else {
    var styl = require("stylus");
  }
  var stylPath = path.join(__dirname, "..", "styl");
  if (stylOpts.includePaths) {
    stylOpts.includePaths.push(stylPath);
  } else {
    stylOpts.includePaths = [stylPath];
  }

  var contents = fs.readFileSync(stylOpts.file, "utf8");
  var css = styl.render(contents.toString("utf-8"));

  fs.writeFileSync("test.txt", css);
  // .done(function(res) {
  //   if (res == null) {
  //     return;
  //   }
  ...
  var parseAssertion = function(rule, ctx) {
    if (rule.type === "comment") {
      ...
      if (startsWith(text, PASS_TOKEN)) {
        ...
        console.log(ctx.currentAssertion);
        console.log(parseFailureDetail);
    ...
    if (rule.type === "comment") {
      if (rule.comment.trim() === EXPECTED_END_TOKEN) {
        ctx.currentAssertion.expected = css.stringify({
          stylesheet: {
            rules: ctx.currentExpectedRules
          }
        });
