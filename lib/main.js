var diffStringsUnified = require("jest-diff").diffStringsUnified;
...
var runStyl = function (stylOptions, trueOptions) {
  ...
  var stylPath = path.join(packagePath, "styl");
  ...
  stylOpts.includePaths = uniq(
    toArray(stylOpts.includePaths).concat([packagePath, stylPath])
  );
  ...
  var css = styl.render(contents.toString("utf-8"), stylOpts);
...
var formatFailureMessage = function (assertion) {
  ...
  msg +=
    "\n\n" +
    diffStringsUnified(assertion.expected || "", assertion.output || "") +
    "\n";
  return msg;
};
...
var parse = function (rawCss, contextLines) {
  ...
  var parseError = function (msg, seeking, pos) {
    var start = pos && pos.start ? pos.start : pos || {};
    ...
    var errorMsg =
      ...
      ".\n" +
      "-- Context --\n" +
      lines.slice(Math.max(0, line - contextLines), line).join("\n") +
      "\n" +
      " ".repeat(column - 1) +
      "^\n";
  ...
  var parseAssertionContainedEnd = function (rule, ctx) {
    if (rule.type === "comment") {
      ...
      if (text === ASSERT_END_TOKEN) {
        ...
        ctx.currentAssertion.expected = ctx.currentExpectedContained.join(
          "\n---\n"
        );
...
var nodePosition = function (node) {
  return node && node.source ? node.source.start : { line: 1, column: 1 };
};
...
var nodeContains = function (outputNode, expectedNode) {
  ...
  if (expectedNode.type === "comment") {
    return outputNode.text.trim() === expectedNode.text.trim();
  }
  ...
  if (expectedNode.type === "rule") {
    return (
      normalizeSpace(outputNode.selector) ===
        normalizeSpace(expectedNode.selector) &&
      childNodesContain(outputNode.nodes, expectedNode.nodes)
    );
  }

  if (expectedNode.type === "atrule") {
    if (
      outputNode.name.toLowerCase() !== expectedNode.name.toLowerCase() ||
      normalizeSpace(outputNode.params) !== normalizeSpace(expectedNode.params)
    ) {
      return false;
    }

    if (expectedNode.nodes) {
      return (
        Boolean(outputNode.nodes) &&
        childNodesContain(outputNode.nodes, expectedNode.nodes)
      );
    }
    ...
    return (
      !outputNode.nodes &&
      stringifyNodes([outputNode]) === stringifyNodes([expectedNode])
    );
  }
...
var startCurrentModule = function (name, ctx) {
  finishCurrentTest(ctx);
...
var finishCurrentModule = function (ctx, pos) {
  finishCurrentTest(ctx);

  if (ctx.useModuleBoundaries) {
    if (pos && !currentModule(ctx)) {
      throw ctx.parseError(
        'Unexpected module end marker "' + END_MODULE_TOKEN + '"',
        "module",
        pos
      );
    }
